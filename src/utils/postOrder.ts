            ...
            if (resp.success === true) {
                ...
                if (isInsufficientBalanceOrAllowanceError(errorMessage)) {
                    ...
                    if (condition === 'sell' || condition === 'merge') {
                        Logger.warning(
                            'For SELL orders, you need to approve conditional tokens. Run: `npm run set-token-allowance`'
                        );
                    } else {
                        Logger.warning(
                            'For BUY orders, check USDC allowance. Run: `npm run check-allowance`'
                        );
                    }
                    Logger.warning('Skipping remaining attempts. Fix allowance before retrying.');
                    break;
                }
                retry += 1;
                Logger.warning(
